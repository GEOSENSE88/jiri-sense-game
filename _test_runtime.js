...
check(document.getElementById('btn-all-modes') !== null, '전체 모드 보기 버튼');
...
console.log('\n=== 🔁 오늘의 도전 ===');
{
  const a = window.eval("JSON.stringify(dailyQueue().map(x=>x.item.accept?x.item.accept[0]:(x.item.q||x.item)))");
  const b = window.eval("JSON.stringify(dailyQueue().map(x=>x.item.accept?x.item.accept[0]:(x.item.q||x.item)))");
  check(JSON.parse(a).length === 10, '일일 도전 10문항');
  check(a === b, '같은 날 동일 문제(결정적 시드)');
...
console.log('\n=== 🏅 업적 ===');
{
  window.eval("ach={}; store.save('geo_ach',ach); xp=0; coins=0; store.save('geo_xp',0); store.save('geo_coins',0); checkAchievements();");
  check(window.eval('!!ach.first') === false, 'XP 0 → 첫 발걸음 미해금');
  window.eval("xp=2000; checkAchievements();");
  check(window.eval('!!ach.first') === true && window.eval('!!ach.rank') === true, 'XP 2000 → 첫 발걸음·지도읽는자 해금');
  check(window.eval('coins') >= 15, '업적 해금 시 보너스 코인');
  check(document.querySelectorAll('#ach-list .ach-item.on').length >= 2, '업적 패널 해금 표시');
  // 이후 코인 테스트 간섭 방지: 전 업적 해금 처리
